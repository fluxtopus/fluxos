...
import { useCapability, useUpdateCapability } from '../../../../../hooks/useCapabilities';
import type { UpdateCapabilityRequest } from '../../../../../types/capability';
...
// Client-side YAML validation (same as create form)
...
export default function EditCapabilityPage() {
  ...
  // Fetch existing capability
  const { capability, isLoading: isLoadingCapability, error: loadError } = useCapability(capabilityId);
  const { update, isLoading: isUpdating, error: updateError } = useUpdateCapability();
  ...
  // Initialize form with existing capability data
  useEffect(() => {
    if (capability && !isInitialized) {
      // Pre-populate YAML from spec_yaml if available
      if (capability.spec_yaml) {
        setYamlContent(capability.spec_yaml);
      }
      // Pre-populate tags
      if (capability.tags) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await update(capabilityId, request);
      // If a new version was created, redirect to the new version's detail page
      // Otherwise, redirect to the same capability's detail page
      router.push(`/capabilities/${response.capability.id}`);
    } catch {
  ...
  // Error state or capability not found
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="bg-[var(--card)] border border-[var(--border)] rounded-lg p-12 text-center">
            <p className="text-lg font-mono text-red-500 mb-4">
              {loadError?.message || 'Capability not found'}
            </p>
            <Link
              href="/capabilities"
  ...
  // Authorization check - only user-owned capabilities can be edited
  if (!capability.can_edit || capability.is_system) {
    return (
      <div className="min-h-screen bg-[var(--background)]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="bg-[var(--card)] border border-[var(--border)] rounded-lg p-12 text-center">
            <LockClosedIcon className="w-12 h-12 text-[var(--muted-foreground)] mx-auto mb-4" />
            <p className="text-lg font-mono text-[var(--foreground)] mb-2">
              Cannot Edit This Capability
            </p>
            <p className="text-sm font-mono text-[var(--muted-foreground)] mb-6">
              {capability.is_system
                ? 'System capabilities cannot be modified.'
                : 'You do not have permission to edit this capability.'}
            </p>
            <Link
  ...
  // No spec_yaml available (shouldn't happen for user-created capabilities)
  if (!capability.spec_yaml) {
    return (
      <div className="min-h-screen bg-[var(--background)]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="bg-[var(--card)] border border-[var(--border)] rounded-lg p-12 text-center">
            ...
            <p className="text-sm font-mono text-[var(--muted-foreground)] mb-6">
              This capability does not have an editable YAML specification.
            </p>
            <Link
              href={`/capabilities/${capabilityId}`}
              className="inline-flex items-center gap-2 px-4 py-2 text-xs font-mono tracking-wider border border-[var(--border)] rounded-md text-[var(--muted-foreground)] hover:bg-[var(--muted)] transition-colors"
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm font-mono text-[var(--muted-foreground)] mt-1">
                {capability.name} <span className="text-xs">v{capability.version}</span>
              </p>
            </div>
          </div>
          <div className="flex items-center gap-3">
            <button
              ...
              disabled={isUpdating || !validationResult?.isValid}
        ...
        <div className="mb-6 p-4 bg-amber-500/10 border border-amber-500/50 rounded-lg">
          <p className="text-sm font-mono text-amber-500">
            <strong>Note:</strong> Modifying the YAML specification will create a new version of this capability.
            ...
            <div className="bg-[var(--card)] border border-[var(--border)] rounded-lg overflow-hidden">
              ...
              <div className="relative">
                <textarea
                  ...
                  spellCheck={false}
                  placeholder="Loading YAML specification..."
                />
              </div>
            </div>
