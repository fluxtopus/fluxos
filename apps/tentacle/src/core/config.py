...
class Settings(BaseSettings):
    ...
    DATABASE_URL: str = os.getenv(
        "DATABASE_URL",
        "postgresql://tentacle:tentacle_pass@postgres:5432/tentacle_db"
    )
    ...
    SITE_NAME: Optional[str] = "Tentacle Multi-Agent System"
...
def validate_secrets() -> None:
    ...
    # Also check the auth_middleware env var (TENTACLE_SECRET_KEY)
    tentacle_secret = os.getenv("TENTACLE_SECRET_KEY")
    ...
    # Check TENTACLE_SECRET_KEY env var used by auth_middleware
    if tentacle_secret is not None:
        if tentacle_secret.lower() in _INSECURE_SECRET_PATTERNS or len(tentacle_secret) < 16:
            issues.append("TENTACLE_SECRET_KEY env var uses an insecure default or is too short (<16 chars)")

    if issues:
        message = (
            "JWT/Secret key validation failed:\n"
            + "\n".join(f"  - {issue}" for issue in issues)
            + "\n\nSet SECRET_KEY and TENTACLE_SECRET_KEY to secure random values "
